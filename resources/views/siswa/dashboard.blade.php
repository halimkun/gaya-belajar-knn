<x-app-layout>
    ...
    <div class="py-12">
        <div class="mx-auto mb-3 max-w-7xl sm:px-6 lg:px-8">
                        ...
                        @if (Auth::user()->assessments->count() > 0)
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-4">
                                    <a href="{{ route('assessments.index') }}" class="rounded-lg bg-indigo-500 px-4 py-2 text-sm text-white hover:bg-indigo-700">Lihat Semua Asesmen</a>
                                    <a href="{{ route('assessments.show', Auth::user()->assessments->sortByDesc('created_at')->first()) }}" class="rounded-lg bg-indigo-500 px-4 py-2 text-sm text-white hover:bg-indigo-700">Lihat Hasil Terakhir</a>
                                </div>
                            </div>
                        @endif
                    @endif
        ...
        @if (Auth::user()->isDetailComplete())
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg border-2 border-amber-500/50">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        @if (Auth::user()->isDetailComplete())
                            @php
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('assessments.create') }}" class="rounded-lg bg-amber-500 px-4 py-2 text-sm text-white hover:bg-amber-700">Isi Asesmen</a>
                            </div>
                            ...
                            @if ($lastAssessment)
                                <div class="mt-4 max-w-xl rounded-lg bg-gray-100 p-4 text-sm text-gray-400 dark:bg-gray-700">
                                    <div class="flex items-center justify-start gap-4">
                                        {{-- icon info --}}
                                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-amber-500 fill-current text-white">
                                        ...
                                        <div>
                                            <p>Assessment terakhir anda: <span class="font-semibold text-amber-600 dark:text-amber-400">{{ $lastAssessment->created_at->diffForHumans() }}</span></p>
                                            <p>Tepatnya pada: <span class="font-semibold text-amber-600 dark:text-amber-400">{{ $lastAssessment->created_at->isoFormat('dddd, D MMMM Y HH:mm') }}</span></p>
                                        </div>
                                    </div>
                                </div>
                            @endif
